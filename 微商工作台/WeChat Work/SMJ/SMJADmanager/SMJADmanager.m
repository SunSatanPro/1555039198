...
@property (nonatomic, getter=isEverLaunched)    BOOL everLaunched;
// 开屏广告
...
- (void)initWithUMAppkey:(NSString *)appkey
    ...
    self.everLaunched = [NSUserDefaults.standardUserDefaults boolForKey:@"SMJADEverLaunched"];
    // 监听网络状态
...
- (void)showRedPacket:(UIViewController *)vc {
    if (self.isOpenAd) {
        [vc presentViewController:SMJADRedPacketViewController.new animated:NO completion:nil];
    }
}
