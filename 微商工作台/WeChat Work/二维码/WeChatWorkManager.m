...
@interface WeChatWorkManager () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureMetadataOutputObjectsDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIGestureRecognizerDelegate, WeChatWorkPreviewViewDelegate>

@property (nonatomic, strong) AVCaptureSession *session;
@property (nonatomic, strong) WeChatWorkPreviewView *previewView;
...
@implementation WeChatWorkManager

#pragma mark - 扫描二维码/条形码

- (instancetype)initWithPreviewView:(WeChatWorkPreviewView *)previewView completion:(nonnull void (^)(void))completion {
    
    self = [super init];
    
    if (self) {
        
        if ([previewView isKindOfClass:[WeChatWorkPreviewView class]]) {
            _previewView = (WeChatWorkPreviewView *)previewView;
...
- (void)stopScanning {
    ...
    [WeChatWorkManager switchTorch:NO];
    [WeChatWorkManager resetZoomFactor];
}
...
#pragma mark - WeChatWorkPreviewViewDelegate
...
- (void)codeScanningView:(WeChatWorkPreviewView *)scanningView didClickedTorchSwitch:(UIButton *)switchButton {
    ...
    [WeChatWorkManager switchTorch:switchButton.selected];
...
+ (UIImage *)generateQRCode:(NSString *)code size:(CGSize)size {
    ...
    UIImage *codeImage = [WeChatWorkManager scaleImage:filter.outputImage toSize:size];
...
+ (UIImage *)generateQRCode:(NSString *)code size:(CGSize)size logo:(nonnull UIImage *)logo {
    ...
    codeImage = [WeChatWorkManager combinateCodeImage:codeImage andLogo:logo];
